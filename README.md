...
The four payloads are deliberately separate derivations — needle edits, `sed` rewrites and verbatim copies respectively, with `hermes-skills` gated more narrowly on purpose — sharing nothing but the `package.nix` shape. Every skill is also packaged on its own, as `packages.<system>.<source>-<skill-name>`: 107 attributes in total, so a consumer installs exactly the skills it wants instead of a whole payload. Each of those is one directory copied out of its gated payload, so it is byte-identical to that skill inside the aggregate and its closure is only its own store path.
