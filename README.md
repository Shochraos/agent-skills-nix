...
| `managed-skills` | self-authored — promoted from oh-my-pi's `~/.omp/agent/managed-skills/`, source lives in this repo's `skills/` tree | 9 |

The three payloads are deliberately separate derivations — needle edits, `sed` rewrites and verbatim copies respectively — sharing nothing but the `package.nix` shape. `packages.<system>.default` is a `symlinkJoin` of all three for single-directory consumers. Each skill in the managed payload is also packaged on its own as `packages.<system>.managed-<skill-name>`; `managed-skills` stays the all-at-once entry point.
...
- `managed-skills` copies the self-authored `skills/` tree verbatim — no upstream input, no rewrites — and runs the same gate set as `vendored-skills`. Promotion means copying a skill out of oh-my-pi's `~/.omp/agent/managed-skills/`; the managed copy is deleted once the payload is live. Every skill is also packaged on its own, generated from a `readDir` of the tree; a single-skill derivation cannot see its siblings, so its `skill://` gate resolves against an ungated copy of the whole tree, which is sound only because these gates never rewrite content.
...
Individual skills in the managed payload install the same way — one `customDirectories` entry per skill, pulling only that skill's files:

```nix
skills.customDirectories = [
  "${inputs.agent-skills-nix.packages.${pkgs.stdenv.hostPlatform.system}.managed-nix-unit-in-flake}"
];
```
...
pkgs/managed-skills/gates.nix         the gates, shared with the per-skill packages
pkgs/managed-skills/per-skill.nix     one package per skill in the managed tree
