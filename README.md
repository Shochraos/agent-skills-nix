...
| `hermes-managed-skills` | self-authored — hermes-agent writes into this repo's `skills/hermes-managed/` tree through its `skills.create_dir` | 3 | `hermes-managed-<skill-name>` |
| `hermes-skills` | [NousResearch/hermes-agent](https://github.com/NousResearch/hermes-agent), one skill from [dbosk/claude-skills](https://github.com/dbosk/claude-skills) and one from [jmiao24/Paper2Agent](https://github.com/jmiao24/Paper2Agent) | 60 | `hermes-<skill-name>` |

The six payloads are deliberately separate derivations — needle edits, `sed` rewrites and verbatim copies respectively, with the two hermes-side payloads gated more narrowly on purpose — sharing only the `package.nix` shape, the gate snippets the two oh-my-pi-side payloads run (`pkgs/lib/skill-gates.nix`) and the installer-pattern list the two hermes-side ones refuse (`pkgs/lib/installer-patterns.nix`). Every skill is also packaged on its own, as `packages.<system>.<source>-<skill-name>`: 120 attributes in total, so a consumer installs exactly the skills it wants instead of a whole payload. Each of those is one directory copied out of its gated payload, so it is byte-identical to that skill inside the aggregate and its closure is only its own store path.
...
- The nine skill sources are flake inputs; the lock is bumped by the workflow, not by hand.
...
A hermes-agent install consumes skills through its own `skills.external_dirs` instead, and a payload directory is a valid entry there. nixfiles points that at a `mkSkillset` selection plus the whole `hermes-managed-skills` payload, so a skill in that tree reaches the agent without further wiring in this repository. nixfiles also sets hermes' `skills.create_dir` to `skills/hermes-managed/` in this checkout, so `skill_manage` creates and edits skills here rather than in a staging directory a promotion step would have to move them out of. nixfiles' build does require a routing bullet for every name under those external dirs in `assets/harness-rules/hermes/SOUL.md` and fails without it, so a new skill lands as a change in both repositories. Skills under `skills/shared/` reach hermes the same way, selected by name from the `shared-skills` payload.
...
pkgs/hermes-skills/package.nix        hermes' bundled catalogue plus two vendored skills, copied verbatim
